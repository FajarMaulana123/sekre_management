...
@section('content')
    ...
    <ol class="breadcrumb float-xl-end">
        ...
        <li class="breadcrumb-item active" aria-current="page"><span>Testimoni</span></li>
    </ol>
    <h1 class="page-header">Testimoni</h1>
       
    <div class="panel panel-inverse">
        <div class="panel-heading">
            <h4 class="panel-title" id="title-bom">Data Testimoni</h4>
                                        ...
                                        <div class="row">
                                            <div class="col-md-12 mb-2">
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" name="perusahaan" id="perusahaan" class="form-control form-control-sm">
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-12 mb-2">
                                                <div class="form-group">
                                                    <label for="">Isi</label>
                                                    <textarea name="isi" id="isi" cols="5" rows="5" class="form-control form-control-sm"></textarea>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
<script>
    $(document).ready(function() {
        ...
        var table = $('#table').DataTable({
            ...
            buttons: [{
                    ...
                    action: function(e, dt, node, config) {
                        ...
                        $('#Modal').modal('show');
                        $('#btn-sb').text('Tambah');
                        $('.judul-modal').text('Tambah Testimoni');
        ...
        $(document).on('click', '.edit', function() {
            ...
            $('#Modal').modal('show');
            $('#btn-sb').text('Update');
            $('.judul-modal').text('Edit Testimoni');
            $('#hidden_status').val('edit');
            $('#hidden_id').val($(this).data('id'));
            $('#nama').val($(this).data('nama'));
            $('#perusahaan').val($(this).data('perusahaan'));
        ...
        $("#add-form").validate({
            ...
            submitHandler: function(form) {
                ...
                if ($('#hidden_status').val() == 'add') {
                    url = '/adm/create_testimoni';
                } else {
                    url = '/adm/update_testimoni';
                }
                $.ajax({
                    url: url,
                    type: "POST",
                    ...
                    success: function(data) {
                        if (data.result != true) {
                            Swal.fire({
                                title: 'Gagal',
                                text: "Gagal Tambah / Update Testimoni",
                                icon: 'error',
                                timer: 3000,
                                showCancelButton: false,
                                showConfirmButton: false,
